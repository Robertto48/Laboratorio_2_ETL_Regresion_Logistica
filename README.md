...
A partir de los resultados obtenidos, el modelo de regresión logística presenta un desempeño consistente entre los conjuntos de test y validación, lo que indica una buena capacidad de generalización. La exactitud (65–66%) muestra que el modelo acierta en una proporción aceptable de predicciones, aunque esta métrica no es la más relevante debido al desbalance de clases. En cuanto a la precisión (37–38%), se observa que el modelo presenta una cantidad considerable de falsos positivos, ya que de todos los usuarios predichos como convertidos, solo una parte realmente lo hace.

Por otro lado, el recall (~69%) evidencia que el modelo logra identificar correctamente la mayoría de los usuarios que efectivamente convierten, siendo esta la métrica más importante en el contexto del negocio, ya que permite captar oportunidades reales de conversión. El F1-score (48–49%) refleja un equilibrio moderado entre precisión y recall, indicando que aún hay margen de mejora en la reducción de falsos positivos. Finalmente, el ROC-AUC (~0.73) demuestra que el modelo tiene una buena capacidad discriminativa, siendo capaz de diferenciar adecuadamente entre usuarios que convierten y los que no. En conjunto, el modelo funciona como una buena base inicial, priorizando la detección de conversiones, aunque requiere ajustes adicionales para mejorar su precisión.
